...
class MyDataset(Dataset):
    ...
    def __init__(self, network_num, fold, positive_ij, negative_ij, mode, dataset):
        ...
        for i in range(network_num):
            A = np.load("heterogeneous_network_A.npy")
            A_encoder = np.load("embedding_Z.npy")
            Ai.append(A)
...
if __name__ == "__main__":
    ...
    positive5foldsidx = np.load(
        "dataset3\index\positive5foldsidx.npy",
        allow_pickle=True,
    )
    ...
    positive_ij = np.load("dataset3\index\positive_ij.npy")
    negative_ij = np.load(r"dataset3\index\negative_ij.npy")
